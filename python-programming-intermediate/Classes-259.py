...
class Dataset:
    ...
    def column(self, label):
        col_idx = self.header.index(label) \
        ...
        col_data = [row[col_idx] for row in self.data]
        return col_data
...
class Dataset:
    ...
    def count_unique(self, label):
        set_col_data = set(self.column(label))
        return len(set_col_data)
...
class Dataset:
    ...
    def __str__(self):
        ...
        for idx in range(min(len(self.data), 10)):
            rows += str(self.data[idx])
        rows += ']'
        return rows
